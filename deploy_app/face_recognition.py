...
haar = cv2.CascadeClassifier(
    os.path.join(base_dir, "model", "haarcascade_frontalface_default.xml")
)  # cascade classifier
model_svm = pickle.load(
    open(os.path.join(base_dir, "model", "model_svm.pickle"), mode="rb")
)  # machine learning model (SVM)
pca_models = pickle.load(
    open(os.path.join(base_dir, "model", "pca_dict.pickle"), mode="rb")
)  # pca dictionary
