...
def genderapp():
    if request.method == "POST":
        ...
        cv2.imwrite(f"./app/static/predict/{pred_filename}", pred_image)
        ...
        for i, obj in enumerate(predictions):
            ...
            matimg.imsave(
                f"./app/static/predict/{gray_image_name}", gray_image, cmap="gray"
            )
            matimg.imsave(
                f"./app/static/predict/{eig_image_name}", eigen_image, cmap="gray"
            )
        ...
        return render_template(
            "gender.html", fileupload=True, report=report
        )  # POST REQUEST
